 ...
 */
const PATH_DROPS: Array<{ match: RegExp; code: string }> = [
  { match: /^\/shop\/?$/, code: 'hidden_shop' },
  { match: /^\/quotes\/?$/, code: 'hidden_quotes' },
  { match: /^\/categories\/?$/, code: 'hidden_categories' },
  { match: /^\/leaderboard\/?$/, code: 'hidden_leaderboard' },
  { match: /^\/authors\/?$/, code: 'hidden_authors' },
];
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        toast.success(`${res.icon ?? '💎'} ${res.title_ar}`, {
          description: `${res.message_ar} (+${res.xp_awarded ?? 0} XP، +${res.coins_awarded ?? 0} 🪙)`,
          duration: 6000,
        });
